...
PlatformLib.prototype.LoadConfig = function loadConfig(config) {
    ...
    .then(config => {
        var configDir = path.join(process.cwd(), ".hx-config");
        ...
        files.forEach(file => {
            try {
                ...
                throw { "error": "Unable to process file: ./.hx-config" + file }
            }
        });
        return config;
    })
    ...
    .then(config => {
        ...
        if (action.subActions) {
            action.subActions.forEach(subActionId => {
                ...
                if (chosenService.dir) {
                    subAction.dir = chosenService.dir;
                }
